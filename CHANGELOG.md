...
- URP from 14.x → 17.x; Cinemachine references updated to 3.x (chase-cam uses `CinemachineCamera + CinemachineThirdPersonFollow`).
- `docs/04_TECHNICAL_ARCHITECTURE.md` rewritten to drop residual v0.1 references to `ClaudeCopilotService` / `AICopilotPersonaSO`; new §7 "Unity 6 compatibility notes"; race flow updated to use LineBank-driven commentator.
- `docs/07_UNITY_SETUP_GUIDE.md` rewritten to v0.2 (no AI proxy / API-key steps; LineBank authoring step); Unity 6 project flow; Edy's-on-Unity-6 note.
